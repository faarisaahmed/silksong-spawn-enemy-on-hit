...
`LocalPaths.props` (gitignored) points at your game install:
...
```sh
cd SpawnEnemyOnHit && dotnet build -c Release
```

The DLL is copied into `BepInEx/plugins/SpawnEnemyOnHit/` automatically.

### Packaging for Thunderstore

```sh
mkdir -p package/BepInEx/plugins/SpawnEnemyOnHit
cp SpawnEnemyOnHit/bin/Release/netstandard2.1/SpawnEnemyOnHit.dll package/BepInEx/plugins/SpawnEnemyOnHit/
cd package && zip -r ../SpawnEnemyOnHit-1.0.0.zip manifest.json icon.png README.md CHANGELOG.md BepInEx
```
